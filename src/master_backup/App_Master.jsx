  ...
  updateGame: (delta) => set((state) => {
    ...
    if (Math.abs(newLastSpawnZ - (-playerZ)) > 30) {
      ...
      if (availableLanes.length > 0) {
        ...
        let type = 'sedan';
        if (typeRoll > 0.9) type = 'truck';
        else if (typeRoll > 0.8) type = 'bus';
        else if (typeRoll > 0.7) type = 'pickup';
        else if (typeRoll > 0.6) type = 'suv';
        else if (typeRoll > 0.5) type = 'ambulance';
        else if (typeRoll > 0.4) type = 'police';
        else if (typeRoll > 0.3) type = 'sport';

        let ownSpeed = 50; // Default slow speed
        if (type === 'truck' || type === 'bus') ownSpeed = 40 + Math.random() * 10; // 40-50 (Very Slow)
        else if (type === 'sedan' || type === 'pickup' || type === 'suv') ownSpeed = 50 + Math.random() * 15; // 50-65 (Medium)
        else if (type === 'sport' || type === 'police') ownSpeed = 65 + Math.random() * 10; // 65-75 (Fast but slower than player min 80)
...
const VEHICLE_DIMENSIONS = {
  ...
  sedan: { width: 1.8, length: 4.0, height: 1.8 },
  truck: { width: 2.8, length: 7.5, height: 3.8 },
  bus: { width: 3.0, length: 8.5, height: 4.0 },
  pickup: { width: 2.4, length: 7.0, height: 2.6 },
  police: { width: 1.8, length: 4.0, height: 1.8 },
  ambulance: { width: 2.8, length: 7.0, height: 3.3 },
  sport: { width: 1.8, length: 4.0, height: 1.8 },
  suv: { width: 2.2, length: 4.5, height: 2.3 }
};
      ...
      {enemies.map(enemy => {
        ...
        return (
          <group key={enemy.id} position={[x, 0, enemy.z]} rotation={[0, 0, tilt]}>
            ...
            {enemy.type === 'pickup' && (
              <group rotation={[0, Math.PI, 0]}>
                {/* Pickup: Truck Model Scaled Down. Scaled up another 15% */}
                <CarModel modelPath="/models/truck.glb" scale={[1.15, 1.03, 0.8]} color="#FF8C00" />
              </group>
            )}
            {enemy.type === 'suv' && (
              <group rotation={[0, 0, 0]}>
                {/* SUV: Ferrari Model Scaled Up (Luxury SUV). Scaled up another 15% */}
                <CarModel modelPath="/models/ferrari.glb" scale={[1.15, 1.72, 1.15]} color="#191970" />
              </group>
            )}
            {enemy.type === 'sedan' && (
              <group rotation={[0, 0, 0]}>
                {/* Sedan: Ferrari Model. Scaled up another 15% */}
                <CarModel modelPath="/models/ferrari.glb" scale={1.15} color="#C0C0C0" />
              </group>
            )}
            {enemy.type === 'police' && (
              <group rotation={[0, 0, 0]}>
                {/* Police: Ferrari Model. Scaled up another 15% */}
                <CarModel modelPath="/models/ferrari.glb" scale={1.15} color="#000000" />
              </group>
            )}
            ...
            {enemy.type === 'sport' && (
              <group rotation={[0, 0, 0]}>
                {/* Sport: Ferrari Model. Scaled up another 15% */}
                <CarModel modelPath="/models/ferrari.glb" scale={1.15} color="#FFFF00" />
              </group>
            )}
          </group>
        );
